...
describe('widget-agent relay — security', () => {
  ...
  it('SSRF guard — allowlist is checked before any fetch call in tool loop', () => {
    ...
    // The fetch call to api.meridian.app must come AFTER the check
    const fetchCallIdx = relay.indexOf("'https://api.meridian.app'", allowlistCheck);
    assert.ok(
      fetchCallIdx > allowlistCheck,
      'fetch() to api.meridian.app must appear after allowlist check',
    );
  });
