...
test.describe('desktop runtime routing guardrails', () => {
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      await runtimeConfig.setSecretValue('MERIDIAN_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, 'wm_test_key_1234567890abcdef');
      ...
      try {
        ...
        await runtimeConfig.setSecretValue('MERIDIAN_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, '');
      }
    });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('meridian.app/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('meridian.app/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/yash-sharma-dev/Meridian/releases/latest';
    ...
    expect(result.linuxFallback).toBe('https://github.com/yash-sharma-dev/Meridian/releases/latest');
  });
  ...
  test('cloud fallback allowed with valid WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      await runtimeConfig.setSecretValue('MERIDIAN_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, testKey);
