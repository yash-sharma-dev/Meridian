...
## Findings
- **`server/_shared/premium-check.ts:10-12`** — `if (keyCheck.valid) return true;` triggers for all meridian.app sessions
- **`api/_api-key.js:49-68`** — `isTrustedBrowserOrigin()` returns `true` for `*.meridian.app`, `*vercel.app`, `localhost`; causes `validateApiKey` to return `{ valid: true, required: false }` with no key present
...
- PR: yash-sharma-dev/Meridian#2386
- Related: `api/_api-key.js:49-68` (`isTrustedBrowserOrigin`)

## Acceptance Criteria
- [ ] Free-tier users on meridian.app do NOT pass `isCallerPremium` check
