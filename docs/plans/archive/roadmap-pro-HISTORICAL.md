...
# Meridian Pro — Implementation Roadmap
...
- **Auth**: none — no Clerk, no sessions. Desktop uses manual `MERIDIAN_API_KEY` in keychain
...
**Description**:
Evaluate and select an authentication provider for Meridian Pro.
...
1. **Meridian Pro Monthly** — $X/mo
2. **Meridian Pro Annual** — $X/yr (discount)
3. **Meridian API Starter** — $Y/mo (1,000 req/day, 5 webhook rules)
4. **Meridian API Business** — $Z/mo (50,000 req/day, unlimited webhooks + SLA)
...
4. Replace ALL `getSecretState('MERIDIAN_API_KEY').present` checks with plan context
...
- [ ] All `getSecretState('MERIDIAN_API_KEY')` references replaced
...
**Description**:
Pro users should NOT need to configure individual API keys for Finnhub, FRED, ACLED, etc. A single Meridian Pro subscription gives access to all 24 services.

**Implementation**:

- Server-side: pro requests use Meridian's own upstream API keys (already configured as env vars)
