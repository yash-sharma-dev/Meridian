...
const ALLOWED_ENV_KEYS = new Set([
  ...
  'OLLAMA_API_URL', 'OLLAMA_MODEL', 'MERIDIAN_API_KEY', 'WTO_API_KEY',
...
function resolveConfig(options = {}) {
  ...
  const remoteBase = String(options.remoteBase ?? process.env.LOCAL_API_REMOTE_BASE ?? 'https://api.meridian.app').replace(/\/$/, '');
  ...
  if (mode === 'docker' && requestedFallback) {
    (options.logger ?? console).warn('[local-api] Cloud fallback disabled in Docker mode (self-hosted instances must not proxy to api.meridian.app)');
  }
