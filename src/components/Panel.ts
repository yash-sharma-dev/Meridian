...
export class Panel {
  ...
  constructor(options: PanelOptions) {
    ...
    if (options.premium && !getSecretState('MERIDIAN_API_KEY').present) {
  ...
  public showLocked(features: string[] = []): void {
    ...
    if (isDesktopRuntime()) {
      ctaBtn.addEventListener('click', () => void invokeTauri<void>('open_url', { url: 'https://meridian.app/pro' }).catch(() => window.open('https://meridian.app/pro', '_blank')));
    } else {
