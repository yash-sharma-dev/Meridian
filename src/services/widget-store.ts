...
// Cookies with domain=.meridian.app are shared across all subdomains
// (meridian.app, tech., finance., commodity., happy., etc.).
// We read cookie first and fall back to localStorage for migration compat.

const COOKIE_DOMAIN = '.meridian.app';
const KEY_MAX_AGE = 365 * 24 * 60 * 60;

function usesCookies(): boolean {
  return location.hostname.endsWith('meridian.app');
}
