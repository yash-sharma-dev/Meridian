...
const DEFAULT_WEB_API_URL = 'https://api.meridian.app';
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('MERIDIAN_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['MERIDIAN_API_KEY']?.value;
        if (wmKeyValue) {
...
export function installWebApiRedirect(): void {
   ...
   *   2. MERIDIAN_API_KEY from runtime config → X-WorldMonitor-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    // MERIDIAN_API_KEY from env or runtime config
    try {
      const { getRuntimeConfigSnapshot } = await import('@/services/runtime-config');
      const wmKey = getRuntimeConfigSnapshot().secrets['MERIDIAN_API_KEY']?.value;
      if (wmKey) {
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.meridian.app/api/...) — just inject auth.
