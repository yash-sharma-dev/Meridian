...
export class DesktopUpdater implements AppModule {
  ...
  private async checkForUpdate(): Promise<void> {
    try {
      const res = await fetch('https://api.meridian.app/api/version', {
      ...
      const releaseUrl = typeof data.url === 'string' && data.url
        ? data.url
        : 'https://github.com/yash-sharma-dev/Meridian/releases/latest';
  ...
  private async resolveUpdateDownloadUrl(releaseUrl: string): Promise<string> {
    try {
      ...
      if (platform) {
        ...
        return `https://api.meridian.app/api/download?platform=${platform}&variant=${variant}`;
      }
    } catch {
